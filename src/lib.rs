...
pub trait Component<X: ComponentExtra = ()>: ToChild<X> {
    ...
    fn update(
        &mut self,
        _context: &mut UpdateContext<X::UpdateContext>,
        event: Event,
    ) -> EventMatch {
        event.m()
    }
    ...
    /// store themselves. This should happen in each component's constructor.
    fn persist(&self, _store: &mut X::PersistentStore) {}
    ...
    fn children(&mut self) -> Vec<Child<'_, X>> {
        Vec::new()
    }
}
...
pub trait ComponentExt<X: ComponentExtra>: Component<X> {
    ...
    fn update_all(
        &mut self,
        context: &mut UpdateContext<X::UpdateContext>,
        event: Event,
    ) -> Option<Event>
    where
        Self: Sized;
    ...
    fn persist_all(&mut self, store: &mut X::PersistentStore)
    where
        Self: Sized;
}

impl<T: Component<X> + ?Sized, X: ComponentExtra> ComponentExt<X> for T {
    fn contains(
        &self,
        context: &UpdateContext<X::UpdateContext>,
        position: Position,
...
pub enum Child<'a, X = ()> {
    ...
    Borrowed {
        name: &'static str,
        component: &'a mut dyn Component<X>,
    },
}
...
impl<'a, X> Child<'a, X> {
    ...
    /// [Child::None].
    pub fn component<'b>(&'b mut self) -> Option<&'b mut dyn Component<X>>
    where
...
/// a trait object.
pub trait ToChild<X> {
    fn to_child(&mut self) -> Child<'_, X>;
}

impl<T: Component<X> + Sized, X: ComponentExtra> ToChild<X> for T {
    fn to_child(&mut self) -> Child<'_, X> {
        Child::Borrowed {
...
// TODO fix conflict
// impl<T: Component<X> + Sized, X: ComponentExtra> ToChild<X> for Option<T> {
//     fn to_child(&mut self) -> Child<'_, X> {
//         match self {
...
fn update_all<X: ComponentExtra>(
    name: &str,
...
fn persist_all<X: ComponentExtra>(
    store: &mut X::PersistentStore,
    component: &mut dyn Component<X>,
) {
...
pub struct ComponentId(AutoIncrementId);
